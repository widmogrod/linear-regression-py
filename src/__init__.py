...
def is_convergent(point, old, convergent):
    return abs(point.x-old.x) < convergent and abs(point.y - old.y) < convergent

def gradient_descent_step(learning_rate, A_point, df):
    return A_point - (df(A_point) * learning_rate)

def gradient_descent(point, learning_rate, convergent, max_iterations, datapoints):
    for i in range(max_iterations):
        old = point
        point = gradient_descent_step(learning_rate, point, gradient_of_least_squers(datapoints))
        if is_convergent(point, old, convergent):
            return point
...
def stochastic_gradient_descent(point, learning_rate, convergent, max_iterations, datapoints):
    for i in range(max_iterations):
        shuffle(datapoints)
        for p in datapoints:
            old = point
            point = gradient_descent_step(learning_rate, point, stochastic_df(p))
...
Y_hat_datapoints_b=list(map(lambda p: (p.x, Y_hat(p.x, B1B2_b.x, B1B2_b.y)), datapoints))
